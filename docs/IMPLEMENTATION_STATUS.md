...
|---|---|---|
| `init` / `status` / `runs` | Implemented | `src/ops/init.rs`, `src/ops/status.rs`, `src/ops/runs.rs`; `tests/m0_integration.rs`; `README.md`, `docs/OPS_WORKFLOW.md`; `diffship init` writes `.diffship/.gitignore`, `.diffship/PROJECT_KIT.md`, `.diffship/PROJECT_RULES.md`, `.diffship/AI_GUIDE.md`, `.diffship/forbid.toml`, and `.diffship/config.toml`, `--lang en|ja` localizes the paste-ready rules snippet, `--refresh-forbid` refreshes only the dedicated forbid file, and the config stub now includes stack-oriented commented `ops.post_apply` presets framed as local normalization |
| `apply` / `verify` / `promote` / `loop` | Implemented (core) | `src/ops/apply.rs`, `src/ops/post_apply.rs`, `src/ops/verify.rs`, `src/ops/promote.rs`, `src/ops/loop_cmd.rs`; `tests/m2_apply_verify.rs`, `tests/m2_promotion_loop.rs`; `README.md`, `docs/OPS_WORKFLOW.md`, `docs/CONFIG.md`; run summaries/TUI now surface command-log counts, phases, and direct log artifact paths from `commands.json`, and `post_apply.json` now also records deterministic changed-path/category summaries for local normalization steps |
| `pack-fix` | Implemented | `src/ops/pack_fix.rs` and automatic generation from `verify` failures in `src/ops/verify.rs`; reprompt zips now also include `run/post_apply.json` plus `run/post-apply/` logs when local post-apply hooks ran, and `PROMPT.md` repeats post-apply changed-path/category evidence before verify-log guidance; covered by `tests/m2_pack_fix.rs` |
