...
|---|---|---|
| `init` / `status` / `runs` | Implemented | `src/ops/init.rs`, `src/ops/status.rs`, `src/ops/runs.rs`; `tests/m0_integration.rs`; `README.md`, `docs/OPS_WORKFLOW.md`; `diffship init` writes `.diffship/PROJECT_KIT.md`, `.diffship/AI_GUIDE.md`, and `.diffship/config.toml` |
...
| Area | Status | Evidence (code / tests / docs) |
|---|---|---|
| `build` command | Implemented | `src/handoff.rs`, `src/cli.rs`, `src/handoff_config.rs`; `tests/m6_handoff_build.rs`; `README.md`; default output naming uses local timestamps and collision suffixes when `--out` is omitted, and either `--out-dir` or `[handoff].output_dir` can redirect the generated bundle under a custom parent directory |
...
| `compare` command (bundle reproducibility check) | Implemented | `src/bundle_compare.rs`, `src/cli.rs`; normalized compare plus `--strict` extracted-entry byte comparison, area/kind diff classification, and `--json` output are covered by `tests/m6_compare.rs`; raw zip container byte equality is intentionally out of the current v1 contract |
