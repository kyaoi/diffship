...
- **S-INIT-002**: It MUST write a human-readable workflow guide derived from `docs/PROJECT_KIT_TEMPLATE.md` (written into `.diffship/` for user attachment) and a separate short `PROJECT_RULES.md` file that is suitable for direct copy/paste into external AI project-rule UIs. The generated workflow guide SHOULD also describe the local failed-run follow-up path, including `diffship strategy` and `diffship pack-fix`.
- **S-INIT-003**: It MUST write a project config stub (e.g., `.diffship/config.toml`) without overwriting existing files unless `--force`.
- **S-INIT-004**: It MUST write an AI-targeted guide derived from `docs/AI_PROJECT_TEMPLATE.md` that explains diffship's workflow, expected artifacts, input file meanings, and non-file deliverables such as commit messages and user-task files, including how failed-run reprompt artifacts relate to local `diffship strategy` inspection and `strategy.resolved.json` when present, and it MUST append current repo metadata such as branch, HEAD, and active local forbid patterns.
