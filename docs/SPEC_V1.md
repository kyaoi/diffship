...
- **S-OUT-001**: Default output is a directory `./diffship_<timestamp>/`, where `<timestamp>` is formatted in the local system timezone as `YYYY-MM-DD_HHMM`; if that path already exists, diffship MUST choose the next available suffixed path (`_2`, `_3`, ...).
