...
## D-056: `diffship init` templates must warn about clean-tree preflight and strict patch-bundle validity

- Date: 2026-03-07
- Decision:
  - Expand `docs/PROJECT_KIT_TEMPLATE.md` and `docs/AI_PROJECT_TEMPLATE.md` so generated guides explicitly warn that `diffship loop` expects a clean target repo working tree.
  - Document that incoming patch bundle zips should be stored outside the repo or under `.diffship/` so the zip itself does not trip the clean-tree guard.
  - Document that ops-compatible bundles must use an exact `base_commit`, must use `apply_mode=git-apply|git-am`, and must not rely on placeholder manifest values.
  - Document that current v1 patch validation refuses binary patches, rename/copy metadata, file mode metadata, and submodule changes.
  - Clarify that `tasks/USER_TASKS.md` is for real user-owned follow-up work, not for repairing an otherwise invalid patch bundle.
- Rationale:
  - The most common first-run failures are contract mismatches that could have been prevented earlier by stronger generated guidance.
  - `diffship init` should make the default operating expectations visible before a user or AI produces an incompatible bundle.
- Implications:
  - `docs/PATCH_BUNDLE_FORMAT.md` should stay aligned with the same strict validator behavior described by the generated guides.
  - Repository-specific prompt templates generated by `init` can now tell AI agents when to fall back to unified diffs or file-by-file edits instead of fabricating an ops bundle.
