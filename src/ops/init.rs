...
# - Put AI-editable defaults and `.diffship/*` edit allowlist entries in `.diffship/ai_generated_config.toml`
# - `.diffship/ai_generated_config.toml` is loaded first; this file keeps final say for scalar defaults
...
# Keep `post_apply` narrow: repository-local normalization only (formatting, docs/spec sync, deterministic fixups).
# Do not use it to compensate for invalid AI artifacts or to hide missing review/verify steps.
# [ops.post_apply]
# Rust-oriented preset:
# cmd1 = "cargo fmt --all"
# cmd2 = "just docs-check"
# cmd3 = "just trace-check"
#
# Node/TS-oriented preset:
# cmd1 = "pnpm exec prettier -w ."
# cmd2 = "pnpm exec eslint . --fix"
#
# Docs/spec-oriented preset:
# cmd1 = "just docs-check"
# cmd2 = "just trace-check"
...
fn default_ai_generated_config_stub(metadata: &GeneratedMetadata) -> String {
    format!(
        r#"# diffship AI-generated local configuration (generated by `diffship init`)
#
# Repository snapshot:
# - repo: {repo_name}
# - current branch: {current_branch}
...
#
# See also: docs/CONFIG.md
# This file is merged as a project-local config layer before `.diffship/config.toml`.
# Keep AI-editable defaults here when you want assistants to adjust local behavior
# without rewriting the user-owned config stub.
#
# Customize this section: opt specific generated `.diffship/*` files into AI editability.
# Only the fixed allowlist below is honored; other `.diffship/*` paths stay forbidden.
...
path6 = "{ai_generated_config}"
# path7 = ".diffship/config.toml" # opt in only if you explicitly want AI to rewrite the user-owned config
#
# Customize this section: add AI-owned defaults that may change over time.
# Example:
# [handoff]
...
"#,
        ...
        current_branch = metadata.branch,
        preferred_target = metadata.preferred_target_branch,
        ai_generated_config = config::AI_GENERATED_CONFIG_RELATIVE_PATH,
    )
}
...
- 組み込みの禁止パスを守る。`.diffship/` では repo が明示的に許可した生成ファイルだけを触り、ローカル `[ops.forbid]` も守る。\n\
